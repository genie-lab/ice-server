...
router.get("/:table/menuList", async (req, res) => {
  const result = await modelCall(boardController.menuList, req);
  res.json(result);
});
...
router.post("/:table/:id/tokenCheck", async (req, res) => {
  const result = await modelCall(boardController.tokenCheck, req);
  res.json(result);
});
...
router.get("/:table/:limit/latest", async (req,res)=>{
  const result = await modelCall(boardController.latest, req);
  res.json(result)
})
//게시물 관련 목록을 가져옴 // 이전글/다음글/관련글
router.get("/:table/:wrGrp/listInfo", async (req,res)=>{
  const result = await modelCall(boardController.listInfo, req);
  res.json(result)
})
//조회수 증가
router.patch("/:table/:id/viewUp", async (req,res)=>{
  const result = await modelCall(boardController.viewUp, req);
  res.json(result)
})
//댓글목록
router.post("/:table/:id/commentList", async (req,res)=>{
  const result = await modelCall(boardController.commentList, req);
  res.json(result)
})
//댓글추가
router.post("/:table/:id/commentAdd", async (req,res)=>{
  const result = await modelCall(boardController.commentAdd, req);
  res.json(result)
})
//댓글수정
router.put("/:table/:id/commentEdit", async (req,res)=>{
  const result = await modelCall(boardController.commentEdit, req);
  res.json(result)
})
//댓글삭제
router.put("/:table/:id/commentDel", async (req,res)=>{
  const result = await modelCall(boardController.commentDel, req);
  res.json(result)
})
//답글추가
router.post("/:table/:id/replyAdd", async (req,res)=>{
  const result = await modelCall(boardController.replyAdd, req);
  res.json(result)
})
//답글수정
router.put("/:table/:id/replyEdit", async (req,res)=>{
  const result = await modelCall(boardController.replyEdit, req);
  res.json(result)
})
//답글삭제
router.put("/:table/:id/replyDel", async (req,res)=>{
  const result = await modelCall(boardController.replyDel, req);
  res.json(result)
})
//파일다운로드
router.get("/:table/:filename/download", async (req,res)=>{
  const result = await modelCall(boardController.download, req);
  res.json(result)
})
